...
; Utiliza R5 e R11 para se comunicar com o programa principal
; R5 indica se houve interrupção e R11 é o contador
GPIOJ_Handler:
        MOV R0, #00000011b ; ACK do bit 0 e 1
        MOV R5, #0000001b ; indica ao programa principal que houve interrupção
        LDR R1, =GPIO_PORTJ_BASE
        LDR R2, [R1, #GPIO_RIS] ; Leitura dos botões no estado anterior
        STR R0, [R1, #GPIO_ICR]
        ...
        ITE EQ       ; se SW1 foi pressionado
          ADDEQ R11, #1
          SUBNE R11, #1 
        AND R11, #0xf ;só considera os 4 bits menos significativos de R11
        
        LDR R2, [R1, #GPIO_IM]
        BIC R2, R2, R0 ; desabilita interrupções
        STR R2, [R1, #GPIO_IM]
...
main:   MOV R0, #(PORTN_BIT | PORTF_BIT)
	...
	LDR R0, =GPIO_PORTJ_BASE
        ...
        MOV R5, #0
        MOV R11, #0
        BL LED_write ;desliga leds
        
loop: 	CBZ R5, espera  ;se não houve interrupção espera
        MOV R0, R11
        BL LED_write
        MOV R5, #0
        MOVT R0, #0x0003 ; constante de atraso
        BL SW_delay  ;debounce
        BL Button_int_enable ; habilita interrupção
